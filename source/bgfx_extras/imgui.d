module bgfx_extras.imgui;

private {
    import bgfx.bgfx : BGFX_HANDLE, invalidHandle, TextureHandle;
...
alias ImGuiAlign = int;             // alignment                            // enum ImGuiAlign_
...
extern(C++, ImGui) @nogc nothrow {
    ...
    void          ShowStyleEditor(ImGuiStyle* ref_ = null);    // style editor block
    ...
    void          PlotLines(const(char)* label, float function(void* data, int idx) values_getter, void* data, int values_count, int values_offset = 0, const(char)* overlay_text = null, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0,0));
    ...
    void          PlotHistogram(const(char)* label, float function(void* data, int idx) values_getter, void* data, int values_count, int values_offset = 0, const(char)* overlay_text = null, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0,0));
    ...
    enum ubyte IMGUI_FLAGS_NONE         = 0x00;
    enum ubyte IMGUI_FLAGS_ALPHA_BLEND  = 0x01;
    
    // Helper function for passing bgfx::TextureHandle to ImGui::Image.
	void Image(TextureHandle _handle
		, ubyte _flags
		, ubyte _mip
		, const(ImVec2) _size
	...
	bool ImageButton(TextureHandle _handle
		, ubyte _flags
		, ubyte _mip
		, const(ImVec2) _size
		, const(ImVec2) _uv0     = ImVec2(0.0f, 0.0f)
...
struct ImGuiIO {
    ...
    const(char)* function() GetClipboardTextFn;
    void function(const(char)* text) SetClipboardTextFn;
...
enum IMGUI_MBUT_RIGHT   = 0x02;
enum IMGUI_MBUT_MIDDLE  = 0x04;

/// For custom values, define these macros before including imgui.h

enum IMGUI_SCROLL_AREA_R = 6;
enum IMGUI_SCROLL_BAR_R = 5;
enum IMGUI_BUTTON_R = 9;
enum IMGUI_BUTTON_RGB0 = imguiRGBA(128, 128, 128, 0);
enum IMGUI_INPUT_R = 4;
enum IMGUI_TABS_HEIGHT = 20;
enum IMGUI_TABS_R = 9;
enum IMGUI_INDENT_VALUE = 16;
enum IMGUI_SEPARATOR_VALUE = 12;
...
uint imguiRGBA(ubyte _r, ubyte _g, ubyte _b, ubyte _a = 255) {
	return 0
		| (cast(uint)(_r) <<  0)
		| (cast(uint)(_g) <<  8)
		| (cast(uint)(_b) << 16)
...
extern(C++) @nogc nothrow {
    ...
    mixin(BGFX_HANDLE!("ImguiFontHandle"));
    ...
    ImguiFontHandle imguiCreate(const(void)* _data = null, uint _size = 0, float _fontSize = 15.0f, AllocatorI _allocator = null);
    void imguiDestroy();

    void imguiBeginFrame(int _mx, int _my, ubyte _button, int _scroll, ushort _width, ushort _height, char _inputChar = 0, ubyte _view = 255);
    void imguiBeginFrame(int _mx, int _my, ubyte _button, int _scroll, ushort _width, ushort _height, ushort _surfaceWidth, ushort _surfaceHeight, char _inputChar = 0, ubyte _view = 255);
    void imguiEndFrame();

    void imguiDrawText(int _x, int _y, ImguiTextAlign.Enum _align, const(char)* _text, uint _argb);
    void imguiDrawLine(float _x0, float _y0, float _x1, float _y1, float _r, uint _argb);
    void imguiDrawRoundedRect(float _x, float _y, float _w, float _h, float _r, uint _argb);
    void imguiDrawRect(float _x, float _y, float _w, float _h, uint _argb);
    ...
    bool imguiBeginArea(const(char)* _name, int _x, int _y, int _width, int _height, bool _enabled = true, int _r = IMGUI_SCROLL_AREA_R);
    void imguiEndArea();
    bool imguiBeginScroll(int _height, int* _scroll, bool _enabled = true);
    void imguiEndScroll(int _r = IMGUI_SCROLL_BAR_R);

    bool imguiBeginScrollArea(const(char)* _name, int _x, int _y, int _width, int _height, int* _scroll, bool _enabled = true, int _r = IMGUI_SCROLL_AREA_R);
    void imguiEndScrollArea(int _r = IMGUI_SCROLL_BAR_R);

    void imguiIndent(ushort _width = IMGUI_INDENT_VALUE);
    void imguiUnindent(ushort _width = IMGUI_INDENT_VALUE);
    void imguiSeparator(ushort _height = IMGUI_SEPARATOR_VALUE);
    void imguiSeparatorLine(ushort _height = IMGUI_SEPARATOR_VALUE, ImguiAlign.Enum = ImguiAlign.Enum.LeftIndented);
    ...
    bool imguiButton(const(char)* _text, bool _enabled = true, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, uint _rgb0 = IMGUI_BUTTON_RGB0, int _r = IMGUI_BUTTON_R);
    bool imguiItem(const(char)* _text, bool _enabled = true);
    bool imguiCheck(const(char)* _text, bool _checked, bool _enabled = true);
    bool imguiBool(const(char)* _text, ref bool _flag, bool _enabled = true);
    ...
    void imguiLabel(uint _rgba, const(char)* _format, ...);
    void imguiValue(const(char)* _text);
    bool imguiSlider(const(char)* _text, float _val, float _vmin, float _vmax, float _vinc, bool _enabled = true, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented);
    bool imguiSlider(const(char)* _text, int _val, int _vmin, int _vmax, bool _enabled = true, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented);
    void imguiInput(const(char)* _label, char* _str, uint _len, bool _enabled = true, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, int _r = IMGUI_INPUT_R);
    ...
    ubyte imguiTabs(ubyte _selected, bool _enabled, ImguiAlign.Enum _align, int _height, int _r, ubyte _nTabs, ubyte _nEnabled, ...);
    ubyte imguiTabs(ubyte _selected, bool _enabled, ImguiAlign.Enum _align, int _height, int _r, ubyte _nTabs, ...);

    uint imguiChooseUseMacroInstead(uint _selected, ...);
    uint imguiChoose(uint _selected, const(char)* args ...) { return imguiChooseUseMacroInstead(_selected, args, null); }

    void imguiColorWheel(float[3] _rgb, bool _respectIndentation = false, float _size = 0.8f, bool _enabled = true);
    void imguiColorWheel(const(char)* _str, ref float[3] _rgb, ref bool _activated, float _size = 0.8f, bool _enabled = true);

    bool imguiImage(TextureHandle _image, float _lod, int _width, int _height, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, bool _enabled = true, bool _originBottomLeft = false);
    bool imguiImage(TextureHandle _image, float _lod, float _scale, float _aspect, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, bool _enabled = true, bool _originBottomLeft = false);
    bool imguiImageChannel(TextureHandle _image, ubyte _channel, float _lod, int _width, int _height, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, bool _enabled = true);
    bool imguiImageChannel(TextureHandle _image, ubyte _channel, float _lod, float _scale, float _aspect, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, bool _enabled = true);
    bool imguiCube(TextureHandle _cubemap, float _lod = 0.0f, ImguiCubemap.Enum _display = ImguiCubemap.Enum.Cross, bool _sameHeight = false, ImguiAlign.Enum _align = ImguiAlign.Enum.LeftIndented, bool _enabled = true);
    ...
    /// some suff for D
    void imguiColorWheel(float _r, float _g, float _b, bool _respectIndentation = false, float _size = 0.8f, bool _enabled = true);
    void imguiColorWheel(const(char)* _text, float _r, float _g, float _b, ref bool _activated, float _size = 0.8f, bool _enabled = true);
}